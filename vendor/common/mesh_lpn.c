...
#include "../Rang_Dong/rd_sensor_tx.h" //RD_EDIT themthu vien
...
const u32 mesh_lpn_key_map[] = {SW1_GPIO, PIR_HI};
#define	MESH_LPN_CMD_KEY		        (PIR_HI)
...
STATIC_ASSERT(LPN_POLL_TIMEOUT_100MS * 100 >= 3000 * 2); // timeout should not be too short.
...
extern u16 LIGHT_SENSOR_TRANS_CNT;
void mesh_friend_ship_start_poll()
{
//	LIGHT_SENSOR_TRANS_CNT=1;
	RD_light_sensor_tx(LIGHT_SENSOR_TRANS_CNT);
...
void lpn_node_io_init()
{
    ...
    #define WAKEUP_LEVEL_LPN    1       // level : 1 (high); 0 (low)
    foreach_arr(i,mesh_lpn_key_map)
    {
      if(!gpio_read(PIR_HI))
      {
		  if(get_provision_state()==STATE_DEV_PROVED||get_provision_state()==STATE_DEV_PROVING){
		   gpio_set_wakeup (mesh_lpn_key_map[1], 1, 1);         // level : 1 (high); 0 (low)
		   cpu_set_gpio_wakeup (mesh_lpn_key_map[1], 1, 1);     // level : 1 (high); 0 (low)
		  }
      }
          gpio_set_wakeup (mesh_lpn_key_map[0], 0, 1);         // level : 1 (high); 0 (low)
          cpu_set_gpio_wakeup (mesh_lpn_key_map[0], 0, 1);     // level : 1 (high); 0 (low)
    }
...
{
	if(!gpio_read(GPIO_PD4)){   //RD_EDIT : sua o day
		...
		foreach_arr(i,mesh_lpn_key_map){
			if(1 == gpio_read (mesh_lpn_key_map[i])){
				key_not_release = 1;
				return mesh_lpn_key_map[i];
			}
		}
...
extern FactoryResetDetect reset_detect ;

//void lpn_key_factory_reset_check()
//{
////	reset_detect.reset_cnt=0;
////	reset_detect.button_press=0;
//    // long press key
//    u32 detkey = 0;
...
//		if(reset_detect.reset_cnt>=4000&&!gpio_read(GPIO_PD4))
//		{
//
//			RD_led_to_debug(0);
//			if(reset_detect.button_press>=5);
//			{
//				irq_disable();
...
//			reset_detect.button_press=0;
//		}
//		if(reset_detect.reset_cnt>500&&reset_detect.reset_cnt<1000&&!gpio_read(GPIO_PD4)){
//			RD_led_to_debug(0);
//			while(!gpio_read(GPIO_PD4));
//			reset_detect.button_press ++;
//			if(reset_detect.button_press>5) reset_detect.button_press=5;
//			//reset_detect.reset_cnt=0;
//			if(get_provision_state() == STATE_DEV_UNPROV){
//			provision_time=0;
//			}
//	    }
//		if(gpio_read(GPIO_PD4)){reset_detect.reset_cnt = 0;RD_led_to_debug(1);}
...
//        }
//    }
//    reset_detect.reset_cnt++;
//}



u32 long_tick ; // sua o day
//u16 factoryReset_cnt=0;
//#define FLASH_ADDR 0x22000
void lpn_key_factory_reset_check()  //TODO : sua ham nay de cua hinh lai thoi gian an pgim
{
    // long press key
    u32 detkey = 0;
    int key_press = 0; // KEY_PRESSED_IDLE
